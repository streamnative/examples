...
- commit offset with metadata: %s <bootstrap-servers> <api-key> <group> <topic> <partition> <offset> "<metadata>"
- show partition offset: %s <bootstrap-servers> <api-key> <group> <topic> <partition>`,
			os.Args[0], os.Args[0])
		os.Exit(1)
	}
	...
	group := os.Args[3]
	topic := os.Args[4]
	partition, err := strconv.Atoi(os.Args[5])
	if err != nil {
