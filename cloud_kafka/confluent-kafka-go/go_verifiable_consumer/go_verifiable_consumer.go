...
func addAssignment(tp kafka.TopicPartition) {
	fmt.Printf("add assignment tp: %v\n", tp)
...
func sendRecordsConsumed(immediate bool) {
	if len(state.currAssignment) == 0 ||
		(!immediate && state.consumedMsgsLastReported+*reportEveryMsgs > state.consumedMsgs) {
		return
	}
...
func doCommit(immediate bool, async bool) {
	if !immediate &&
		(state.autoCommit ||
			state.consumedMsgsAtLastCommit+*reportEveryMsgs > state.consumedMsgs) {
		return
	}
...
func handleMsg(m *kafka.Message) bool {
	...
	for a == nil {
		...
		addAssignment(m.TopicPartition)
		a = findAssignment(m.TopicPartition)
	}
...
func mainLoop(c *kafka.Consumer, timeout int, reportInterval int) {
	tmout := time.NewTicker(time.Duration(timeout) * time.Second)
	commitTicker := time.NewTicker(time.Duration(reportInterval) * time.Millisecond)
...
func runConsumer(config *kafka.ConfigMap, topic string, reportInterval int) {
	...
	mainLoop(c, 10*60000, reportInterval)
...
func main() {
	...
	apiKey := kingpin.Flag("api-key", "API key").Required().String()
	sessionTimeout := kingpin.Flag("session-timeout", "Session timeout").Required().Int()
	reportInterval := kingpin.Flag("report-interval-time", "Report interval (ms)").Required().Int()
	reportEveryMsgs = kingpin.Flag("report-interval-msgs", "Report every # msgs").Default("1000").Int()
	verbosity = kingpin.Flag("verbosity", "Verbosity level").Default("1").Int()
	...
	conf["sasl.username"] = "unused"
	conf["sasl.password"] = "token:" + *apiKey
	...
	runConsumer((*kafka.ConfigMap)(&conf), *topic, *reportInterval)
