...
# Delete hydra default access token
docker run --rm \
...
# Replace hydra access token
docker run --rm \
  --network hydra_default \
  -v "$(pwd)"/kop/java/src/main/resources/hydra/keys:/tmp/keys \
  oryd/hydra:v1.11.7 \
  --endpoint=http://hydra:4445 \
  keys import hydra.jwt.access-token /tmp/keys/private_key.json /tmp/keys/public_key.json

# Create a new client, the client id is simple_client_id and secret is simple_client_secret
# Correspond pulsar role is simple_client_id
docker run --rm \
  --network hydra_default \
  oryd/hydra:v1.11.7 \
  clients create \
